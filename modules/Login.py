...
logindb = mysql.connector.connect(
    host="localhost",
    user="Euazaki",
    password="ImperiKrieg_27th",
    database = "loginDB"
)

mycursor = logindb.cursor()

# Create the remembered_users table if it doesn't exist
mycursor.execute("""
...
class LogInFrame(ctk.CTkFrame):
    ...
    # Check if the "Remember me" checkbox is checked
        if self.check_var.get():
            # Get the ID number and password from the input fields
            id_number = self.id_entry.get().strip()
            password = self.pass_entry.get().strip()
            
            # Call the remember_user method to store the user's credentials in the database
            self.remember_user(id_number, password)
        else:
    ...
    # Remember the user function via SQL database
    def remember_user(self, id_number, password):
        mycursor.execute("SELECT * FROM remembered_users WHERE ID_number = %s", (id_number,))
        if mycursor.fetchone() is None:
            mycursor.execute("INSERT INTO remembered_users (ID_number, password) VALUES (%s, %s)", (id_number, password))
            logindb.commit()
        else:
            mycursor.execute("UPDATE remembered_users SET password = %s WHERE ID_number = %s", (password, id_number))
            logindb.commit()
    ...
    # Auto login for the remember me system
    def auto_login(self):
        mycursor.execute("SELECT * FROM remembered_users")
        remembered_users = mycursor.fetchall()
