    ...
    if request.method == "POST":
        ...
        if not all([class_id, subject, semester, teacher_id, status]):
            ...
            try:
                ...
                cursor.close()
                conn.close()
                 # Redirect to admin UI after successful addition
            except mysql.connector.Error as err:
                error = f"Error: {err}"
...
def new_password():
    if request.method == 'POST':
        ...
        if id_number is None:
            return jsonify({'success': False, 'message': "Session has expired. Please try again."}), 400
        ...
        try:
            ...
            return jsonify({'success': True, 'message': "Your password has been successfully updated."}), 200
        except mysql.connector.Error:
            return jsonify(
                {'success': False, 'message': "Cannot connect to the database. Please try again later."}), 500
    return redirect(url_for('changepasswordcomplete'))
